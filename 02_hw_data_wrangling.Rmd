...
There are 7,014 candidates in this dataset. 
...
knitr::kable(party_incumbent_promote, 
             digits = 3, 
             col.names = c("Party", "Incumbency Status", 
                           "Average Promo Proportion"))
party_incumbent_promote
```


Democratic incumbent candidates sponsored the most promoting ads on average. 
...
plot_q3 <- ggplot(us_house_ads, aes(x =impressions_millions)) +
  geom_histogram() + 
  labs(title = "Facebook Ad Impressions of 
       US House Candidates",
...
```
The historgram is almost an "L" shape in the sense that it has a very steep peek 
at the left and sharply decreases as the number of impressions goes on. 
The historgram illustrates how very few US House Candidates received more than 
10 million ad impressions on Facebook
...
fb_top_plot <- ggplot(top_candidates, aes(x = impressions_millions,
                                          y = cand_name_last,
                                          fill = party)) +
  geom_bar(stat = "identity") +
  labs(title = "Top 15 Candidate Facebook Ad Impressions, 
       2017-2018",
